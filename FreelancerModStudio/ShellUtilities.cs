...
public class SysImageListHelper
{
    ...
    ImageList.ImageCollection smallImageCollection = null;
    ImageList.ImageCollection largeImageCollection = null;
}
...
public static class ShellUtilities
{
    ...
    const int SHGFI_ICON               = 0x00100;     // get icon
    const int SHGFI_DISPLAYNAME        = 0x00200;     // get display name
    const int SHGFI_TYPENAME           = 0x00400;     // get type name
    const int SHGFI_ATTRIBUTES         = 0x00800;     // get attributes
    const int SHGFI_ICONLOCATION       = 0x01000;     // get icon location
    const int SHGFI_EXETYPE            = 0x02000;     // return exe type
    const int SHGFI_SYSICONINDEX       = 0x04000;     // get system icon index
    const int SHGFI_LINKOVERLAY        = 0x08000;     // put a link overlay on icon
    const int SHGFI_SELECTED           = 0x10000;     // show icon in selected state
    const int SHGFI_ATTR_SPECIFIED     = 0x20000;     // get only specified attributes
    const int SHGFI_LARGEICON          = 0x00000;     // get large icon
    const int SHGFI_SMALLICON          = 0x00001;     // get small icon
    const int SHGFI_OPENICON           = 0x00002;     // get open icon
    const int SHGFI_SHELLICONSIZE      = 0x00004;     // get shell size icon
    const int SHGFI_PIDL               = 0x00008;     // pszPath is a pidl
    const int SHGFI_USEFILEATTRIBUTES  = 0x00010;     // use passed dwFileAttribute
	//if (_WIN32_IE >= 0x0500)
    const int SHGFI_ADDOVERLAYS        = 0x00020;     // apply the appropriate overlays
    const int SHGFI_OVERLAYINDEX       = 0x00040;     // Get the index of the overlay

    const int FILE_ATTRIBUTE_NORMAL    = 0x00080;     // Normal file
    const int FILE_ATTRIBUTE_DIRECTORY = 0x00010;     // Directory
    ...
    static extern IntPtr ShellExecute(int hwnd, string lpOperation, string lpFile, 
        string lpParameters, string lpDirectory, int nShowCmd);

    [DllImport("shell32.dll", CharSet = CharSet.Auto)]
    static extern IntPtr SHGetFileInfo(string pszPath, int dwFileAttributes,
